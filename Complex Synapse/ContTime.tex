...
\begin{equation}\label{eq:multistateineq}
  \begin{aligned}
    \fptb_{ik} - \fptb_{jk} &=
      ...
      \end{cases} \\
    \eq_i \W^F_{ij} \prn{\fptb_{ij}+\fptb_{ji}} &= 1 \quad\text{if}\quad i=j\pm1,
  \end{aligned}
\end{equation}
%
...
%  \implies c_k =&\, \frac{\prn{k-\frac{n+1}{2}} - \sum_j\prn{j-\frac{n+1}{2}}\eq_j}{f^+f^-},
  \implies c_k =&\, \frac{k - \sum_j j\eq_j}{f^+f^-},
\end{aligned}
\end{equation}
%
...
\begin{equation}\label{eq:multistatearea}
 % A = 2\sqrt{N} \sum_k \brk{\prn{k-\frac{n+1}{2}} - \sum_j\prn{j-\frac{n+1}{2}}\eq_j} \eq_k \w_k.
  A = 2\sqrt{N} \sum_k \brk{k - \sum_j j\eq_j} \eq_k \w_k
    = 2\sqrt{N} \sum_k \abs{k - \sum_j j\eq_j} \eq_k ,
\end{equation}
%
...
\footnote{Note that including the dropped factors in \eqref{eq:noisepm} would only change this to $A \leq \sqrt{N(4\eq_1\eq_n)}(n-1)$, which would have no effect on what follows.}

%
\begin{equation}\label{eq:multistateextarea}
  A \leq \sqrt{N}(n-1)\prn{4\eq_1\eq_n}.
\end{equation}
%
