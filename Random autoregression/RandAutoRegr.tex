...
%\begin{align}
%\label{eq:rssol}
  q &= \frac{\brk{ -(\alpha-1)\opo + \sqrt{ (\alpha-1)^2\opo^2 + (2\alpha-1)\omo^2 } }}{\omo^2},\\
%\label{eq:zmsol}
  \zeta_- &= \frac{1+\abs{\omega}^2-q\inv}{2\omb},\\
%\label{eq:phisol}
  \Phi(\omega) &= \frac{1}{4\pi}\brk{ (1-\alpha)\ln q -\alpha\ln\prn{\abs{\frac{\omega}{\zeta_-}}} -\ir\alpha\arg(-\omega)},
%\end{align}
\end{aligned}
\end{equation}
%%
...
This means that it has a rotation symmetry.
