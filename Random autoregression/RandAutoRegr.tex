...
%\label{eq:phisol}
  \Phi(\omega) &= \frac{1}{4\pi}\brk{ (1-\alpha)\ln q -\alpha\ln\prn{\frac{\omega}{\zeta_-}} },
%\end{align}
\end{aligned}
\end{equation}
%%
...
\end{equation}
%
%which satisfies \eqref{eq:zmreq} everywhere.
...
In evaluating determinants in \sref{sec:simplederiv}, we will come across contour integrals of the form
%
...
The branch cuts and contour are shown in \fref{fig:contours}(\ref{fig:branches}).
...
For the first part, we can use the original contour $C$ as in \fref{fig:contours}(\ref{fig:contout}).
...
For the second part, we can deform the contour to $C'$ as in \fref{fig:contours}(\ref{fig:contin}):
%
...
The second and fourth would cancel if not for the discontinuity in the logarithm.
This leaves:
%
...
\begin{aligned}
  \oint_{C'} \frac{\dr\zeta}{2\pi\ir\,\zeta}\, \ln\prn{\frac{\zeta-\zeta_<}{\zeta}}
    &= \int_{\theta}^{\theta+2\pi}\frac{\dr\phi}{2\pi} \ln\prn{\frac{\epsilon\e^{\ir\phi}-\zeta_<}{\epsilon\e^{\ir\phi}}}
     + \int_{\epsilon}^{\abs{\zeta_<}-\epsilon} \frac{\dx}{2\pi\ir\,x} \operatorname{disc}\ln\prn{\frac{x\e^{\ir\theta}-\zeta_<}{x\e^{\ir(\theta+2\pi)}}}\\
