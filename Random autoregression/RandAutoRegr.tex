...
First, $\alpha\to1$:
%
\begin{equation}\label{eq:rsato1}
  rs \to \pm\frac{1}{(\alpha-1)\omo},
\end{equation}
%
which always satisfies \eqref{eq:rsreq},
%
\begin{equation}\label{eq:zmato1}
...
\end{multline}
%
both of which satisfy \eqref{eq:rsreq},
%
\begin{equation}\label{eq:zmatoinfty}
  \implies \zeta_- = \frac{1+\abs{\omega}^2}{2\omb}
  \quad\text{or}\quad 
  \frac{\omega}{\alpha\opo} \brk{1 + \frac{2\abs{\omega}^2}{\alpha\opo^2} + \CO(\alpha^{-2})},
\end{equation}
%
only the second of which satisfies \eqref{eq:zmreq}
%
\begin{equation}\label{eq:phiatoinfty}
  \implies \Phi = -\frac{\ln\opo}{4\pi} + \CO(\alpha^{-1}),
\end{equation}
%
...
\begin{equation}\label{eq:saddlereq}
  \abs{\zeta_-}\leq1,
  \qquad
  [\alpha^2+rs\opo]^2 \geq 4(rs)^2\abs{\omega}^2,
\end{equation}
%
in which case
%
...
\end{equation}
%
With the aid of this expression for $rs$, the second condition \eqref{eq:saddlereq} reduces to
%
\begin{equation}\label{eq:rsreqsimple}
  rs \leq \frac{\alpha^2(2\alpha^3-2\alpha+1)}{2(\alpha+1)(\alpha-1)^3\opo}.
\end{equation}
%
