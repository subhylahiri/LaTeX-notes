...
\newcommand{\cc}{^\ast}
\newcommand{\hc}{^\dag}
\newcommand{\pcc}{^{\prime\ast}}
\newcommand{\phc}{^{\prime\dag}}
\newcommand{\csq}{^{\ast2}}
\newcommand{\hsq}{^{\dag2}}
...
\newcommand{\pmut}{^{\prime\mu\mathrm{T}}}
\newcommand{\pmuc}{^{\prime\mu\ast}}
...
\newcommand{\alpham}{\abs{\alpha}}
\newcommand{\betam}{\abs{\beta}}
\newcommand{\chim}{\abs{\chi}}
\newcommand{\alphatm}{\abs{\alphat}}
\newcommand{\betatm}{\abs{\betat}}
\newcommand{\chitm}{\abs{\chit}}
\newcommand{\sh}{\hat{s}}
%
%================================================================================
\begin{document}
...
\begin{table}
  \centering
  \begin{tabular}{CAAAAAC}
  \toprule
    \text{Matrix}  & \colheadc{Similarity} & \colheadc{Doubling} & \colheadc{Constraint} 
        & \colheadc{Choice} & \colheadc{Result} & \text{Ambiguity} \\
  \midrule
\Gammab & -&\Gammab^\mu &         \Gammab^2 &= \gamma \I  &          &-      & \gamma &= 1 &       &-       & \ZZ    \\
    \A  &  &\Gammab\mud &      \A\inv \A\hc &= \alpha \I  &  \alpham &= 1    & \alpha &= 1 &       &-       & \R     \\
    \At & -&\Gammab\mud &    \At\inv \At\hc &= \alphat \I & \alphatm &= 1    &        &-   &       &-       & \Cb\cc \\
    \B  &  &\Gammab\muc &          \B \B\cc &= \beta \I   &    \beta &\in \R & \betam &= 1 & \beta &= \pm1  & U(1)   \\
    \Bt & -&\Gammab\muc &        \Bt \Bt\cc &= \betat \I  &   \betat &\in \R &        &-   &       &-       & \Cb\cc \\
    \C  &  &\Gammab\mut &   \C\inv \C\trans &= \chit \I   &   \chi^2 &= 1    &        &-   &  \chi &= \pm1  & \Cb\cc \\
    \Ct & -&\Gammab\mut & \Ct\inv \Ct\trans &= \chit \I   &  \chit^2 &= 1    &        &-   & \chit &- \pm1  & \Cb\cc \\
  \bottomrule
  \end{tabular}
  ...
  The result must be proportional to the identity matrix, and the constant of proportionality is defined there.
  ...
  The sixth column contains the results of imposing the fifth column on the fourth.
  The seventh column contains the remaining scalar ambiguity that the first column may be multiplied by after imposing the fifth column.
  }\label{tab:involconstraints}
\end{table}
...
\begin{table}
  ...
  \toprule
    \text{Matrix}  & \colheadc{Similarity} & \colheadc{Doubling} & \colheadc{Constraint} 
        & \colheadc{Choice} & \colheadc{Result} & \text{Ambiguity} \\
  \midrule
    \At & -&\Gammab\mud & \A \Gammab \At\inv &= a \I  & \alphat &= \alpha a'/a^* &    &-   & \alphat &= a      & -               \\
        &&&            \Gammab\hc \A \At\inv &= a' \I & \abs{a} &= \abs{a'}      & a' &= 1 & \abs{a} &= 1      & \R\times\ZZ     \\
    \Bt & -&\Gammab\muc &\Bt\Gammab\cc\B\inv &= b \I  &  \betat &= \beta b^* b'  &    &-   &  \betat &= b\beta & -               \\
        &&&               \Bt \B\inv \Gammab &= b' \I &  b^* b' &\in \R          & b' &= 1 &       b &\in \R   & U(1)\times\ZZ   \\
    \Ct & -&\Gammab\mut & \C \Gammab \Ct\inv &= c \I  &   \chit &= \chi c/c'     &    &-   &   \chit &= c\chi  & -               \\
        &&&         \Gammab\trans \C \Ct\inv &= c' \I &     c^2 &= c\psq         & c' &= 1 &       c &=  \pm1  & \Cb\cc\times\ZZ \\
  \bottomrule
  \end{tabular}
  \caption{Constants associated with chirality, charge conjugation, \etc matrices.
  The second column lists the result of the matrix in the first column being used in a similarity transform on \(\Gammab^\mu\).
  ...
  The fourth column contains constraints from \cref{eq:constinvol}.
  The fifth column contains normalisation choices from \cref{eq:normscalar}.
  The sixth column contains the results of imposing the fifth column on the fourth, including the same column of \cref{tab:involconstraints}.
  The seventh column contains the remaining scalar ambiguity that the first column may be multiplied by after imposing the fifth column.
  The rows go in pairs and should be read together.
  The constraints follow by substituting into the corresponding rows of \cref{tab:involconstraints}.
  the first scalar ambiguity is applied to both matrices of the corresponding rows in \cref{tab:involconstraints}, 
  the second ambiguity is applied to the matrices in first column and \(\Gammab\).
  }\label{tab:chiralconstraints}
\end{table}
...
\begin{table}
  ...
  \begin{tabular}{AAAAA}
  \toprule
    \colheadc{Similarity} & \colheadc{Doubling} & \colheadc{Constraint} 
        & \colheadc{Choice} & \colheadc{Result} \\
  \midrule
    &\Gammab\mud &     \C\cc(\A\B)\inv &= f \I    &   f' &= f\cc\alpha\beta\cc\chi & f' &= 1 &  f &= \beta\chi \\
    &&              \A\inv \B\trans \C &= f' \I   &      &-                        &    &-   &    &-      \\
    %
    &&               \Ct\cc(\A\Bt)\inv &= f_1 \I  &  f_1 &= f / b c^*  &    &- &  f_1 &= f/bc \\
    &&            \A\inv \Bt\trans \Ct &= f_1' \I & f_1' &= f' b' / c' &    &- & f_1' &= 1    \\
    %
   -&\Gammab\mud &   \Ct\cc(\At\B)\inv &= f_2 \I  &  f_2 &= f a'/c\pcc &    &- &  f_2 &= f    \\
    &&            \At\inv \B\trans \Ct &= f_2' \I & f_2' &= f' a / c   &    &- & f_2' &= a/c  \\
    %
    &&               \C\cc(\At\Bt)\inv &= f_3 \I  &  f_3 &= f a / b'   &    &- &  f_3 &= fa   \\
    &&            \At\inv \Bt\trans \C &= f_3' \I & f_3' &= f' a' b    &    &- & f_3' &= b    \\
  \bottomrule
  \end{tabular}
  \caption{Constants associated with chirality, charge conjugation, \etc matrices.
  The first column lists the result of a similarity transform on \(\Gammab^\mu\).
  The second column lists the matrix that performs the operation from the first column twice.
  The result must be proportional to the identity matrix, and the constant of proportionality is defined there.
  The third column contains constraints from \cref{eq:constinvol}.
  The fourth column contains normalisation choices from \cref{eq:normscalar}.
  The fifth column contains the results of imposing the fourth column on the third, including the corresponding column of \cref{tab:involconstraints,tab:chiralconstraints}.
  The first two rows form a pair and should be read together.
  The constraints follow from substituting the second column into one of the rows of \cref{tab:involconstraints}. 
  The remaining constraints follow from substituting one of the rows of \cref{tab:chiralconstraints} into the second column.
  }\label{tab:comboconstraints}
\end{table}
...
\begin{table}
  ...
  \toprule
    \text{Matrix}  
            & \colhead{Similarity} & \colhead{Nonexistent}& \text{Doubling}
...
\begin{equation*}
  \Lambda_\mu{}^\nu \S\inv_\Lambda \Gammab^\mu \S_\Lambda 
    ...
    = \Gammab^\nu.
  % \quad \text{and}\quad
  % \Lambda_\mu{}^\nu \St\inv_{-\Lambda} \Gammab^\mu \St_{-\Lambda} 
  %   = \Lambda_\mu{}^\nu (-\Lambda^\mu{}_\rho) (-\Gammab^\rho)
  %   = \Gammab^\nu.
\end{equation*}
%
...
\Cref{tab:exist} then tells us that whenever \(\S_\Lambda\) does not exist, \(\St_{-\Lambda}\) does not exist either.
...
But their real and imaginary parts, \(\thetab_a,\phib_a\), do come from spinors \(\Thetab_a,\Phib_a\):
%
\begin{equation*}
\begin{aligned}
...
\end{equation*}
%
In \(\R^{2n}\) we have \(2n\) linearly independent vectors, \(\thetab_a,\phib_a\)
...
Therefore any real vector \(\psib\) can be written as a linear combination of the \(\thetab_a,\phib_a\) with \emph{real} coefficients.
...
\end{equation}
%
Note that if \(\Gammab^\mu\) is similar to the first form, then \(-\Gammab^\mu\) is similar to the second form, and vice versa. 
