...
\begin{figure}
  ...
  %\input{.TpX}
  \caption{(a-b)Example tracks. Runs: Coloured, thin lines, reorientations: black, thin lines. Omega turns and reversals labelled as $\Omega$ and $R$. Grouping of turns into reorientations indicated by reo 1, reo 2, etc. 
...
\end{equation}
%
The lower bound is saturated when one of the variables is enough to determine the others, it only makes sense for discrete distributions. The upper bound is saturated when the $X_i$ are independent:
%
...
\begin{figure}
  ...
  %\input{.TpX}
  \caption{Estimating animal-to animal variability. (a) probability of $\Omega$ turns for individual animals and the weighted mean. (b) probability of Right turns for individual animals and the weighted mean. For weighted mean, errorbar is the square root of $\sigma_p$ \eqref{eq:varinot}. }\label{fig:variability}
\end{figure}
