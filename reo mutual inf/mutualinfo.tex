...
\end{equation}
%
Then, $\bar{p}$ and $\sigma_p$ can be calculated with \eqref{eq:varinot}.
...
Now we will restrict attention to binary cases, \ie for reorientation type we only look at two types: omega and reversal-omega. For reorientation angle we only pay attention to the sign, \ie whether it is to the right or to the left. The results are in \fref{fig:binaryresults}.
